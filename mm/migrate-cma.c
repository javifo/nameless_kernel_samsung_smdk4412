					...
					int pass, int tries)
{
	...
	if (!mapping) {
		...
		rc = fallback_migrate_page(mapping, newpage, page, mode, pass, tries);
...
static int __unmap_and_move(struct page *page, struct page *newpage,
				int force, bool offlining, enum migrate_mode mode, int pass, int tries)
{
	int rc = -EAGAIN;
...
int migrate_replace_cma_page(struct page *oldpage, struct page **newpage)
{
	...
	ret = migrate_pages(&cc.migratepages, __migrate_replace_alloc,
			    (unsigned long)newpage, false, MIGRATE_SYNC, 0);
